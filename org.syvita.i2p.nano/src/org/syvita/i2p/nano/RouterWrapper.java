...
public class RouterWrapper {
	...
	public void updateBandwidthLimitKBps(int n) {
		...
		if (!saved) {
			throw new RuntimeException("Error saving the new bandwidth limit");
		}
	...
	public double get1sRateInKBps() {
		try {
			return router.getContext().bandwidthLimiter().getReceiveBps()/1024d;
		} catch (Exception e) {
			return 0;
		}
	}
	public double get1sRateOutKBps() {
		try {
			return router.getContext().bandwidthLimiter().getSendBps()/1024d;
		} catch (Exception e) {
			return 0;
		}
	}
	...
	public double getTotalInMB() {
		try { 
			return router.getContext().bandwidthLimiter().getTotalAllocatedInboundBytes() / 1048576d;
		} catch (Exception e) {
			return 0;
		}
	}
	public double getTotalOutMB() {
		try {
			return router.getContext().bandwidthLimiter().getTotalAllocatedOutboundBytes() / 1048576d;
		} catch (Exception e) {
			return 0;
		}
	}
	...
	public static class NetworkStateMessage {
		...
		NetworkStateMessage(NetworkState state, String message) {
			setMessage(state, message);
		}

		public void setMessage(NetworkState state, String message) {
			this.state = state;
			...
			switch (status) {
				...
				default:
					...
					if (ra == null && _context.router().getUptime() > 5*60*1000) {
						...
						} else if (_context.getProperty(PROP_I2NP_NTCP_HOSTNAME) == null || _context.getProperty(PROP_I2NP_NTCP_PORT) == null) {
