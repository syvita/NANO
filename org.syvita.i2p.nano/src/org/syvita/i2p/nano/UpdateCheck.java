...
public class UpdateCheck {
	...
	private static boolean isNewerVersionAvailable(String currentVersion, String versionAvailable) {
		if (versionAvailable == null || versionAvailable.trim().equals("")) {
			return false;
		} 
	...
	public static String lookupVersionAvailable(Router router) {
		try {
			...
			if (status.equals("HTTP/1.1 200 OK")) {
				...
				if (rawVersionAvailable.length() > 10) {
					return null;
				}
				...
				for(var index = 0; index < rawVersionAvailable.length(); index++) {
					if (allowedChars.indexOf(rawVersionAvailable.charAt(index)) >= 0) {
						sanitizedVersionAvailable += rawVersionAvailable.charAt(index);
					}
				}
			}
